...
// -------------------------------------------------------------
// Wrapper for PostCSS (single file)
// -------------------------------------------------------------
...
function runPostCSSBatch(patternRel, outDirRel, minify = false) {
    ...
    // record all files generated inside outDirRel
    ...
    allFiles.forEach(f => {
        metrics.files.push({
            file: f.rel,
            time: t1 - t0,    // batch time applies equally
            size: f.size
        });
    });

    console.log(`✔ ${outDirRel} (batch ${minify ? "min" : "std"}) — ${fmt(t1 - t0)}`);
}

// recursively gather CSS files inside folder
function collectCSSFiles(abs, relBase) {
    const out = [];
    const entries = fs.readdirSync(abs, {withFileTypes: true});
    for (const e of entries) {
        const pAbs = path.join(abs, e.name);
        const pRel = path.join(relBase, e.name);
        if (e.isDirectory()) {
...
// -------------------------------------------------------------
// Build components/themes (batch mode)
// -------------------------------------------------------------
