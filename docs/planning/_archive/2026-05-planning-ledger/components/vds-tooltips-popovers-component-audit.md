# VDS Tooltips and Popovers Component Audit
...
Source item: `VDS-2310`

Next recommended item: `VDS-2320 Typography component audit`

This file records the tooltips/popovers component audit for the component modernization wave. It is a planning artifact only: no CSS, raw docs, generated docs metadata, generated `dist`, package metadata, workflows, selector inventory, consumer reports, npm tags, or version fields change here.

## Policy Summary

- `@24vlh/vds/src/components/tooltips-popovers.css` is the source truth for current tooltip/popover CSS behavior.
- `@24vlh/vds/doc-raw/vds-tooltips-popovers.doc.html` and `@24vlh/agents/docs_vds/components/vds-tooltips-popovers.json` are docs/index evidence.
- Current tooltip/popover CSS provides visual/placement/state hooks for tooltip triggers, non-interactive tooltip surfaces, popover anchors, interactive popover shells, placement modifiers, arrow controls, density variants, semantic variants, and open/visible state hooks.
- Consumer/application code owns trigger semantics, `aria-describedby`, `aria-expanded`, `aria-controls`, `aria-haspopup`, open/close state, focus management, Escape/outside-click dismissal, collision-aware positioning, portal/mount strategy, touch fallback, and interactive popover keyboard behavior.
...
- No selector rewrite, docs rewrite, behavior implementation, generated index refresh, selector inventory update, or generated output refresh happens in `VDS-2310`.

## Source CSS Evidence

`@24vlh/vds/src/components/tooltips-popovers.css` currently has:
...
- Runtime and composition hooks such as `.tooltip--visible`, `.tooltip--interactive`, `.popover--open`, `[data-state="open"]`, placement classes, arrow classes, and semantic variants must remain compatibility-sensitive until later approved migration work.
