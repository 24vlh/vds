...
- Master item: `VDS-2290`
- Plan file: `@24vlh/vds/docs/planning/features/VDS-2290-tabs-component-audit.md`

## 1. Goal

Create the tabs component audit for the `VDS-0500 Component Modernization Wave`. This item records the current CSS-only tabs contract: tablist/tab/panel structure, active/hidden/disabled hooks, underline/pill/segmented variants, density and alignment modes, vertical orientation, icon-only tabs, icon/badge slots, scrollable tab lists, responsive vertical collapse, docs/index metadata, package-facing dist presence, and accessibility/theming/motion risks before any tabs CSS fixes or docs rewrites begin.

## 2. Scope

### In scope

- Record current `src/components/tabs.css` selector, local variable, tablist, tab, panel, variant, density, orientation, scroll, responsive, focus, disabled, theme, and accessibility evidence.
- Record public and candidate-public selector inventory evidence for the tabs class surface.
- Record raw docs and generated docs-index metadata for `vds-tabs`.
- Record package-facing `dist/components/tabs.css` and `.min.css` presence.
- Add a tabs component audit artifact for later tabs CSS fixes, docs rewrite, selector classification, APG tabs guidance, keyboard/focus policy, responsive QA, forced-colors cleanup, theme/contrast checks, migration notes, and release verification.
- Update the master feature map so `VDS-0500` remains in progress, `VDS-2290` is done, and the next recommended item is `VDS-2300 Toasts component audit`.
...
- Adding tab activation logic, roving focus, keyboard handling, panel lifecycle behavior, routing, loading behavior, ARIA synchronization, accessibility smoke tests, visual checks, selector reclassification, docs rewrites, or generated output.
...
- Repo files:
  - `@24vlh/vds/src/components/tabs.css`
  - `@24vlh/vds/doc-raw/vds-tabs.doc.html`
  - `@24vlh/vds/dist/components/tabs.css`
  - `@24vlh/vds/dist/components/tabs.min.css`
...
- `@24vlh/vds/src/components/tabs.css` has `398` lines.
- Selector blocks: `55`.
